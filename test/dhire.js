...
contract('DHire', ([deployer, author]) => {
  ...
  describe('dhire', async () => {
    ...
    it('register and get user', async () => {
      await dhire.registerUser('', '', { from: author }).should.be.rejected;
      
      await dhire.registerUser(name1, bio1, { from: author }).should.not.be.rejected;
      const user = await dhire.getUser(name1);
      assert.equal(user['0'], name1);
      assert.equal(user['1'], bio1);
      assert.equal(user['2'].toNumber(), 0);

      await dhire.registerUser(name1, bio1, { from: author }).should.be.rejected;
    });
    
    it('upload resume and get resume', async () => {
      await dhire.uploadResume(docHash, { from: author }).should.not.be.rejected
    ...
    it('send and get messages', async () => {
      await dhire.sendMessage(name1, "", { from: author }).should.be.rejected
      await dhire.sendMessage("", message1, { from: author }).should.be.rejected
      await dhire.sendMessage(name1, message1, { from: author }).should.not.be.rejected
      await dhire.sendMessage(name1, message2, { from: author }).should.not.be.rejected

      const messages = await dhire.getMessages({ from: author });
      const m1 = messages[0].split(':'), m2 = messages[1].split(':');
